...
#define FUN_ARG_EX_() \
    FUN_ARG_6, FUN_ARG_5, FUN_ARG_4, FUN_ARG_3, FUN_ARG_2, FUN_ARG_1, FUN_ARG_0
...
typedef struct {
    ...
    uintptr values[LOG_MULTI_PARA_MAX];
} HiLogContent;
#pragma pack()
...
typedef enum {
    /* Output to the UART without buffer. Commercial versions are forbidden. */
    OUTPUT_OPTION_DEBUG = 0,
...
#define HILOG_DEBUG(mod, fmt, ...) \
    HiLogPrintf(mod, HILOG_LV_DEBUG, FUN_ARG_NUM(__VA_ARGS__), fmt, ##__VA_ARGS__)
#else
#define HILOG_DEBUG(mod, fmt, ...)
#endif
...
#define HILOG_INFO(mod, fmt, ...) \
    HiLogPrintf(mod, HILOG_LV_INFO, FUN_ARG_NUM(__VA_ARGS__), fmt, ##__VA_ARGS__)
#else
#define HILOG_INFO(mod, fmt, ...)
#endif
...
#define HILOG_WARN(mod, fmt, ...) \
    HiLogPrintf(mod, HILOG_LV_WARN, FUN_ARG_NUM(__VA_ARGS__), fmt, ##__VA_ARGS__)
#else
#define HILOG_WARN(mod, fmt, ...)
#endif
...
#define HILOG_ERROR(mod, fmt, ...) \
    HiLogPrintf(mod, HILOG_LV_ERROR, FUN_ARG_NUM(__VA_ARGS__), fmt, ##__VA_ARGS__)
#else
#define HILOG_ERROR(mod, fmt, ...)
#endif
...
#define HILOG_FATAL(mod, fmt, ...) \
    HiLogPrintf(mod, HILOG_LV_FATAL, FUN_ARG_NUM(__VA_ARGS__), fmt, ##__VA_ARGS__)
#else
#define HILOG_FATAL(mod, fmt, ...)
#endif
