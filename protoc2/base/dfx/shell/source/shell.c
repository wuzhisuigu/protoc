...
extern const ShellCommand *msg_cmd_list[];
extern int msg_cmd_count;

extern void shellSendMsg(Shell *shell, const ShellCommand* command, int argc, char** argv);
...
void shellInit(Shell *shell, char *buffer, unsigned short size)
{
	...
	shell->msgList.base = msg_cmd_list;
	shell->msgList.count = msg_cmd_count;

    shellAdd(shell);

#if SHELL_SUPPORT_USER
...
static const char* shellGetCommandName(ShellCommand *command)
{
    ...
    else if (command->attr.attrs.type <= SHELL_TYPE_KEY)
    {
        shellToHex(command->data.key.value, buffer);
        return buffer;
    } else if (command->attr.attrs.type <= SHELL_TYPE_MSG)
    {
        return command->data.msg.name;
    } else {
        return "";
    }
...
void shellListItem(Shell *shell, ShellCommand *item)
{
    ...
    else if (item->attr.attrs.type <= SHELL_TYPE_MSG)
    {
        shellWriteString(shell, shellText[SHELL_TEXT_TYPE_MSG]);
    }
...
ShellCommand* shellSeekMsg(Shell *shell,
                               const char *cmd,
                               ShellCommand **msg,
                               unsigned short compareLength)
{
    const char *name;
    unsigned short count = shell->msgList.count -
        ((uintptr_t)msg - (uintptr_t)shell->msgList.base) / sizeof(ShellCommand*);
    for (unsigned short i = 0; i < count; i++)
    {
        name = shellGetCommandName(msg[i]);
        if (!compareLength)
        {
            if (strcmp(cmd, name) == 0)
            {
...
#endif
#if SHELL_SUPPORT_MSG
    else if (command->attr.attrs.type == SHELL_TYPE_MSG)
    {
        shellSendMsg(shell, command, shell->parser.paramCount, shell->parser.param);
    }
...
void shellExec(Shell *shell)
{
    ...
    if (shell->status.isChecked)
    {
        ...
        ShellCommand *command = shellSeekCommand(shell,
                                                 ...
                                                 0);
        if (command == NULL) {
			command = shellSeekMsg(shell,
                                       shell->parser.param[0],
                                       (ShellCommand**)shell->msgList.base,
                                       0);
		}
...
void shellTab(Shell *shell)
{
    ...
    ShellCommand *lastMachedCmd = NULL;
    ...
    else if (shell->parser.length > 0)
    {
        ...
        shell->parser.buffer[shell->parser.length] = 0;
        ShellCommand *base = (ShellCommand *)shell->commandList.base;
        ShellCommand **msg = (ShellCommand **)shell->msgList.base;
        for (i = 0; i < shell->commandList.count; i++)
        {
                        ...
                        == shell->parser.length)
            {
                ...
                lastMatchIndex = i;
                lastMachedCmd = &base[i];
                matchNum++;
            }
        }
        if (matchNum == 0)
        { 
			for (i = 0; i < shell->msgList.count; i++)
        	{
        	    if (shellCheckPermission(shell, msg[i]) == 0
        	        && shellStringCompare(shell->parser.buffer,
        	                           (char *)shellGetCommandName(msg[i]))
        	                == shell->parser.length)
        	    {
        	        if (matchNum != 0)
        	        {
        	            ...
        	            shellListItem(shell, msg[lastMatchIndex]);
        	            length = 
        	                shellStringCompare((char *)shellGetCommandName(msg[lastMatchIndex]),
        	                                   (char *)shellGetCommandName(msg[i]));
        	            maxMatch = (maxMatch > length) ? length : maxMatch;
        	        }
        	        lastMatchIndex = i;
        ...
        if (matchNum != 0)
        {
            shell->parser.length = 
                shellStringCopy(shell->parser.buffer,
                                (char *)shellGetCommandName(lastMachedCmd));
        }
        if (matchNum > 1)
        {
            shellListItem(shell, lastMachedCmd);
            shellWritePrompt(shell, 1);
...
int shellExit(int argc, char *argv[])
{
    Shell *shell = shellGetCurrent();
    if (!shell->exit) {
        return 0;
...
SHELL_EXPORT_CMD(
SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN)|SHELL_CMD_DISABLE_RETURN,
help, shellExit, exit from current session\r\n);

int shellPs(int argc, char *argv[])
{
    Shell *shell = shellGetCurrent();
    if (!shell->ps) {
        return 0;
...
SHELL_EXPORT_CMD(
SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN)|SHELL_CMD_DISABLE_RETURN,
help, shellPs, show all thread info\r\n);
...
int shellRun(Shell *shell, const char *cmd)
{
    ...
    if (strlen(cmd) > (unsigned int)(shell->parser.bufferSize - 1))
    {
        shellWriteString(shell, shellText[SHELL_TEXT_CMD_TOO_LONG]);
        return -1;
    }
