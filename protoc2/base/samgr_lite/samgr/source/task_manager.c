...
TaskPool *SAMGR_CreateFixedTaskPool(const TaskConfig *config, const char *name, uint8 size)
{
    ...
    if (queueId == NULL)
    {
        LOGE("Create Queue<%s> size:%d failed!", name, config->queueSize);
        return NULL;
    }

    TaskPool *taskPool = (TaskPool *)MEM_Malloc(sizeof(TaskPool) + sizeof(ThreadId) * size);
    if (taskPool == NULL)
    {
        LOGE("Create TaskPool<%s> size:%d failed!", name, config->queueSize);
        QUEUE_Destroy(queueId);
        return NULL;
    }

    (void)memset(taskPool, 0, sizeof(TaskPool) + sizeof(ThreadId) * size);
...
int32 SAMGR_StartTaskPool(TaskPool *pool, const char *name)
{
    ...
    while (pool->top < pool->size)
    {
        ...
        if (threadId == NULL)
        {
            LOGE("Start Task<%s, %d, %d> failed!", name, pool->stackSize, pool->priority);
            break;
        }
...
static void *TaskEntry(void *argv)
{
    ...
    while (TRUE)
    {
        ...
        if (exchange.type == (uint16)MSG_EXIT)
        {
...
static void EndWork(ServiceImpl *service, const Exchange *exchange)
{
    ...
    if (interval > MSG_PROC_THRESHOLD)
    {
        ...
        LOGI("Message Timeout <service:%s, feature:%d, type:%d, reqId:%d, time:%ums>", name,
             exchange->id.featureId, exchange->type, exchange->request.msgId, interval);
