...
#define UTILS_DL_LIST_LAST(object) ((object)->pstPrev)
    ...
    /* insert node before item */
    static inline void UtilsListInsert(UTILS_DL_LIST *next, UTILS_DL_LIST *node)
    {
        UTILS_DL_LIST *prev = next->pstPrev;
        node->pstPrev = prev;
        node->pstNext = next;
        next->pstPrev = node;
        prev->pstNext = node;
 ...
 * @see
 */
#define UTILS_OFF_SET_OF(type, member) ((uintptr) & ((type *)0)->member)
